...
export default async function NovoPedidoPage({
  ...
  const [{ data: customers }, { data: sizes }, { data: combos }, profitMode] = await Promise.all([
    ...
    supabase
      .from("recipe_sizes")
      .select(
        "id, size_label, fixed_price, fixed_price_monthly, recipe_id, recipes(name, pet_id, pets(id, name)), recipe_size_ingredients(id, quantity, unit, ingredient_id, ingredients(id, name, unit, price_per_unit, loss_pct))"
      )
      .order("size_label"),
    supabase
      .from("combos")
      .select(
        "id, name, fixed_price, discount_pct, combo_items(id, quantity, recipe_size_id, recipe_sizes(id, size_label, fixed_price, recipe_id, recipes(name, pet_id, pets(id, name)), recipe_size_ingredients(id, quantity, unit, ingredient_id, ingredients(id, name, unit, price_per_unit, loss_pct))))"
      )
      .order("name"),
    getProfitCalcMode(),
  ]);
